...
class MDXSeparator(CommonSeparator):
    ...
    def load_model(self):
        ...
        if self.segment_size == self.dim_t:
            ...
            ort_inference_session = ort.InferenceSession(
                self.model_path, providers=self.onnx_execution_provider, sess_options=ort_session_options
            )
    ...
    def separate(self, audio: np.ndarray) -> np.ndarray:
        """
        Separates the audio into vocal stem based on the model's configuration.
        It processes the mix, demixes it into sources, normalizes the sources, and returns the output.

        Args:
            audio (numpy array): The audio to be processed.
        ...
        self.logger.debug(f"Preparing mix for input audio...")
        mix = self.prepare_mix(self.audio)
